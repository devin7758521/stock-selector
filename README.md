...
6. **评星**：LLM 侧为 **0～5 星**（**0 星 = 无星**：加权综合分**低于**内部门槛时为 0 星，默认阈值为 **32 分**，可在 `analyzer_enhanced.py` 中调整 `EnhancedLLMAnalyzer.WEIGHTED_SCORE_ZERO_STAR_BELOW`）。**全市场五星最多 2 只**（按当日排序保留前 2，其余降为四星并改写理由前缀）。**AI 插件五星**同样最多 2 只（见 `selector.py`）。  

...
class StockSelector:
    ...
    def _load_or_filter_sectors(self):
        ...
        if os.path.exists(json_path):
            try:
                ...
                if saved_date == today:
                    # 优先用涨幅前5（由run_sector.py实时获取并存入）
                    top_sectors = data.get("top_sectors", [])
                    if top_sectors:
                        logger.info(f"从 sector_results.json 加载今日涨幅前5板块: {[s['name'] for s in top_sectors]}")
                        return top_sectors
                    # 其次用周K筛选结果
                    sectors = data.get("sectors", [])
                    if sectors:
                        logger.info(f"从 sector_results.json 加载 {len(sectors)} 个周K筛选板块（{saved_date}）")
                        return sectors
                    logger.info("sector_results.json 今日数据为空，实时筛选")
                else:
    ...
    def run(self) -> List[Dict]:
        ...
        if self.sector_results:
            ...
            # 将板块数据注入LLM插件，修复板块联动分析
            for plugin in self.plugin_manager.get_active_plugins():
                if hasattr(plugin, "sector_results"):
                    plugin.sector_results = self.sector_results
        ...
        # Step 3.75: 预抓市场/宏观新闻（全局只抓一次，注入 LLM 插件）
        # ── 原因：市场/宏观新闻与个股无关，若放在每只股票的 process_stock 里抓，
        #    N 只股票 = N 次重复请求（全打到东方财富，GitHub Actions 上还被封）。
        #    在这里预抓一次，通过 cached_market_ctx / cached_macro_ctx 注入插件，
        #    每只股票只需抓自己的个股新闻，大幅减少网络请求和 akshare 超时等待。
        logger.info("Step 3.75: 预抓市场/宏观新闻（全局缓存）...")
        try:
            from screener.news_akshare import (
                _search_market_news_list, _search_macro_news_list, _format_news_context
            )
            _mkt_news  = _search_market_news_list(days=3, max_results=10)
            _macro_news = _search_macro_news_list(days=3, max_results=5)
            _cached_market_ctx = _format_news_context(_mkt_news,  "市场财经新闻") if _mkt_news  else ""
            _cached_macro_ctx  = _format_news_context(_macro_news, "宏观政策新闻") if _macro_news else ""
            logger.info(
                f"市场新闻预抓完成: {len(_mkt_news)} 条，宏观新闻: {len(_macro_news)} 条"
            )
        except Exception as e:
            logger.warning(f"预抓市场/宏观新闻失败，各股将自行抓取（降级）: {e}")
            _cached_market_ctx = None   # None = 不传缓存，各股自行抓取
            _cached_macro_ctx  = None

        # 将缓存注入所有 LLM 插件
        for plugin in self.plugin_manager.get_active_plugins():
            if hasattr(plugin, "cached_market_ctx"):
                plugin.cached_market_ctx = _cached_market_ctx
                plugin.cached_macro_ctx  = _cached_macro_ctx
                logger.debug(f"市场/宏观新闻缓存已注入插件 {plugin.name}")

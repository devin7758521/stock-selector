...
class TrendAnalysisResult:
    ...
    # 情绪分析
    sentiment_score: float = 50.0    # 情绪分 0-100
    sentiment_signal: str = ""        # 情绪信号描述

    # 五星评级
    star_rating: int = 0             # 星级 0-5
    star_display: str = ""           # 星级显示
...
class StockTrendAnalyzer:
    ...
    def analyze(self, df: pd.DataFrame, code: str) -> TrendAnalysisResult:
        ...
        # 9. 五星评级
        self._calculate_star_rating(result)
    ...
    def _generate_signal(self, result: TrendAnalysisResult) -> None:
        ...
        if bias != bias or bias is None:  # NaN or None defense
            bias = 0.0
        base_threshold = self.BIAS_THRESHOLD
    ...
    def _analyze_sentiment(self, result: TrendAnalysisResult) -> None:
        """
        情绪分析
        ...
        """
        # 基于技术指标计算情绪分
        sentiment_score = 50.0  # 基础分
        
        # 趋势强度贡献（0-30分）
        trend_contribution = (result.trend_strength / 100) * 30
        sentiment_score += trend_contribution
        ...
        volume_scores = {
            ...
            VolumeStatus.NORMAL: 10,
            VolumeStatus.SHRINK_VOLUME_UP: 5,     # 无量上涨较差
            VolumeStatus.HEAVY_VOLUME_DOWN: 0,    # 放量下跌最差
        }
        volume_contribution = volume_scores.get(result.volume_status, 10)
        sentiment_score += volume_contribution
        
        # MACD状态贡献（0-20分）
        macd_scores = {
            MACDStatus.GOLDEN_CROSS_ZERO: 20,  # 零轴上金叉最强
            MACDStatus.GOLDEN_CROSS: 15,      # 金叉
            MACDStatus.CROSSING_UP: 12,       # 上穿零轴
            MACDStatus.BULLISH: 10,            # 多头
            MACDStatus.BEARISH: 5,            # 空头
            MACDStatus.CROSSING_DOWN: 2,       # 下穿零轴
            MACDStatus.DEATH_CROSS: 0,        # 死叉
        }
        macd_contribution = macd_scores.get(result.macd_status, 5)
        sentiment_score += macd_contribution
        
        # RSI状态贡献（0-15分）
        rsi_scores = {
            RSIStatus.OVERSOLD: 15,       # 超卖最佳
            RSIStatus.STRONG_BUY: 12,     # 强势
            RSIStatus.NEUTRAL: 8,        # 中性
            RSIStatus.WEAK: 5,            # 弱势
            RSIStatus.OVERBOUGHT: 2,       # 超买最差
        }
        rsi_contribution = rsi_scores.get(result.rsi_status, 5)
        sentiment_score += rsi_contribution
        
        # 乖离率调整（-5到+5分）
        if -2 < result.bias_ma5 < 2:
            sentiment_score += 5  # 价格贴近MA5，情绪积极
        elif result.bias_ma5 > 5:
            sentiment_score -= 5  # 乖离率过高，情绪谨慎
        elif result.bias_ma5 < -5:
            sentiment_score -= 3  # 乖离率过低，情绪恐慌
        
        # 支撑位调整（0-5分）
        if result.support_ma5 or result.support_ma10:
            sentiment_score += 5  # 获得支撑，情绪稳定
        ...
        if sentiment_score >= 80:
            result.sentiment_signal = "极度乐观"
        elif sentiment_score >= 65:
            result.sentiment_signal = "乐观"
        elif sentiment_score >= 50:
            result.sentiment_signal = "中性"
        elif sentiment_score >= 35:
            result.sentiment_signal = "谨慎"
        else:
            result.sentiment_signal = "悲观"
    
    def _calculate_star_rating(self, result: TrendAnalysisResult) -> None:
        """
        五星评级
        ...
        result.star_display = "⭐" * star_rating
        
        # 生成评级理由
        reasons = []
        if star_rating >= 4:
            reasons.append(f"情绪分高达{sentiment_score:.1f}/100，市场情绪极度乐观")
        elif star_rating >= 3:
            reasons.append(f"情绪分{sentiment_score:.1f}/100，市场情绪乐观")
        
        if result.trend_status in [TrendStatus.STRONG_BULL, TrendStatus.BULL]:
            reasons.append(f"{result.trend_status.value}，趋势强劲")
        
        if result.macd_status in [MACDStatus.GOLDEN_CROSS_ZERO, MACDStatus.GOLDEN_CROSS]:
            reasons.append("MACD金叉，动能向上")
        
        if result.volume_status == VolumeStatus.SHRINK_VOLUME_DOWN:
            reasons.append("缩量回调，洗盘充分")
        
        if result.support_ma5 or result.support_ma10:
            reasons.append("获得均线支撑，回调到位")
        
        if reasons:
            result.rating_reason = "；".join(reasons)
        else:
            result.rating_reason = "市场情绪中性，技术指标无明显优势"
    ...
    def format_analysis(self, result: TrendAnalysisResult) -> str:
        ...
        lines = [
            ...
            f"",
            f"😊 情绪分析:",
            f"   情绪分: {result.sentiment_score:.1f}/100",
            f"   情绪信号: {result.sentiment_signal}",
            f"",
            f"⭐ 五星评级:",
            f"   评级: {result.star_rating}星",
            f"   星级显示: {result.star_display}",
            f"   评级理由: {result.rating_reason}",
            f"",
